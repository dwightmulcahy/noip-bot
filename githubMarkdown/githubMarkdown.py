...
class GithubMarkdown:
    ...
    def __wrap(self, text, mdwrapper, level=0) -> str:
        ...
        if isinstance(text, list):
            return '\n'.join([self.__wrap(l, mdwrapper, level + 1) for l in text])
    ...
    def listordered(self, text, level=(-1)):
        ...
        if isinstance(text, list):
            return ''.join([self.listordered(l, level+1) for l in text])

    def listunordered(self, text, level=(-1)):
        if isinstance(text, str):
            return f'{level*"    "}* {text}\n'
        if isinstance(text, list):
            return ''.join([self.listunordered(l, level+1) for l in text])
    ...
    def checklist(self, text, level=(-1)):
        if isinstance(text, str):
            return f'{level*"    "}- [ ] {text}\n'
        if isinstance(text, list):
            return ''.join([self.checklist(l, level+1) for l in text])
