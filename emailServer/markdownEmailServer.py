...
class MarkdownEmailServer:
    def __init__(self, appName: str, gmailSender: str, gmailToken: str, debug=False):
        ...
        if debug:
            logging.info("Gmail server enabled in DEBUG MODE.")
            self.gmail = GMail(f"{appName} <{gmailSender}>", gmailToken)
        else:
            logging.info("Gmail server enabled.")
            self.gmail = GMailWorker(f"{appName} <{gmailSender}>", gmailToken)

    @staticmethod
    def githubMarkdown(text: str) -> str:
        return markdown(
            str(text), extensions=githubTypeExt, extension_configs=githubTypeExtConfig
        )
    ...
    def __send(
        self, sendTo: str, subject: str = "", textBody: str = "", htmlBody: str = ""
    ):
